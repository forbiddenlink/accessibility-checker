...
vi.mock("@/utils/imageAnalyzer", () => {
  return {
    ImageAnalyzer: function MockImageAnalyzer(this: {
      analyzeImages: typeof mockAnalyzeImages;
    }) {
      this.analyzeImages = mockAnalyzeImages;
    },
  };
});
...
describe("/api/analyze-images", () => {
  ...
  describe("edge cases specific to image analysis", () => {
    ...
    it("handles images with aria-label instead of alt", async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.results[0].accessibility.ariaLabel).toBe(
        "Image description via aria-label",
      );
    });
  });
});
