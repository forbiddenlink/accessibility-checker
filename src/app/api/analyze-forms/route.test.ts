...
describe("/api/analyze-forms", () => {
  ...
  describe("edge cases specific to form analysis", () => {
      ...
      mockAnalyzeForms.mockResolvedValueOnce([
        {
          ...
          issues: [
            ...
            {
              code: "FIELD_NO_LABEL",
              message: "text field lacks a proper label",
              severity: "error",
            },
            {
              code: "FIELD_NO_LABEL",
    ...
    it("handles forms with different severity levels", async () => {
      mockAnalyzeForms.mockResolvedValueOnce([
        {
          ...
          issues: [
            ...
            {
              code: "FORM_ENHANCEMENT",
              message: "Consider adding autocomplete attribute",
              severity: "info",
            },
          ],
        },
      ...
      expect(response.status).toBe(200);
      const severities = data.results[0].issues.map(
        (i: { severity: string }) => i.severity,
      );
      ...
      mockAnalyzeForms.mockResolvedValueOnce([
        {
          ...
          method: "post",
          fields: [
            { type: "text", hasLabel: true, required: false, ariaLabels: true },
          ],
          issues: [
            {
              code: "FORM_NO_IDENTIFIER",
