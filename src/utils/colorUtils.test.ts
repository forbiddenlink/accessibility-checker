...
describe("colorUtils", () => {
  ...
  describe("findClosestAccessibleColor", () => {
    it("finds closest WCAG compliant color", () => {
      ...
      const result = findClosestAccessibleColor(
        "#777777",
        "#000000",
        4.5,
        "WCAG",
...
describe("generateColorSuggestions accuracy", () => {
  // The UI renders `contrast` as a measured "4.50:1", so it must be the
  // ratio of the colour actually returned, not the target it aimed for.
  it("reports the measured ratio of the suggested colour, not the target", () => {
    const suggestions = generateColorSuggestions(
      "#757575",
      "#FFFFFF",
      4.6,
    ...
    for (const s of suggestions) {
      ...
      const actual = getContrastRatio(
        getLuminance(fg.r, fg.g, fg.b),
        getLuminance(bg.r, bg.g, bg.b),
      );
      expect(s.contrast).toBeCloseTo(actual, 2);
    }
  });

  it("picks black over white on a mid-grey background", () => {
    // bg #808080: white gives 3.95:1 (fails AA), black gives 5.32:1.
    const suggestions = generateColorSuggestions(
      "#8A8A8A",
      "#808080",
      1.1,
    ...
    const optimal = suggestions.find((s) =>
      s.description.startsWith("Maximum contrast"),
    );
    expect(optimal?.foreground).toBe("#000000");
    expect(optimal!.contrast).toBeGreaterThan(4.5);
  });

  it("does not label a sub-threshold APCA suggestion as AA", () => {
    // #FFFFFF on #949494 measures Lc ~62.5, below the Lc 75 body minimum.
    const suggestions = generateColorSuggestions(
      "#9A9A9A",
      "#949494",
      5,
    ...
    expect(Math.abs(optimal!.contrast)).toBeLessThan(75);
    expect(optimal!.level).not.toBe("AA");
    expect(optimal!.level).not.toBe("AAA");
  });
});
